#ifndef _FMCSM_CUDA_LAPLACE_2D_KERNEL_H_
#define _FMCSM_CUDA_LAPLACE_2D_KERNEL_H_

#include "nmc/FMCSM_CUDA2DKernel.h"
...
namespace nmc
{
	class FMCSM_CUDA2D_Kernel :
		public FMCSM_CUDA2DKernel
	{
	public:
		///////////////////////////////////////////////
		/*!
			initialize kernel
		*/
		void Init(FMCSM_CUDA2DTree* tree);
		...
		*/
		void ElementMPCefficients(
			const FMCSM_CUDA2DTree* tree,
			FMCSM_CUDA2DTreeNode* node,
			const float2* c_vec);
		...
		*/
		void EvalMultipole(
			const FMCSM_CUDA2DTree* tree,
			const FMCSM_CUDA2DTreeNode* node,
			float2* dst);
	...
	__host__ __device__ static inline double binomial_coefficient(int n, int m)
	{
	...
	void FMCSM_CUDA2D_Kernel::Init(
		nmc::FMCSM_CUDA2DTree *tree)
	{
	}

	__global__ void Laplace2D_P2M_Kernel(
		int term_num,
		float cell_size,
		float min_center_x,
		float max_center_y,
	...
	void FMCSM_CUDA2D_Kernel::ElementMPCefficients(
		const FMCSM_CUDA2DTree* tree,
		FMCSM_CUDA2DTreeNode *node,
		const float2* c_vec)
	{
		...
		int grid_size = 16384; //power of 2
		int block_size = num_of_cell / grid_size;
		if(num_of_cell < grid_size){
			grid_size = num_of_cell;
			block_size = 1;
		}
		dim3 Dg(grid_size, 1, 1);
		dim3 Db(block_size, 1, 1);
		Laplace2D_P2M_Kernel<<<Dg, Db, 2*(tree->m_TermNum+1)*block_size*sizeof(float)>>>(
			tree->m_TermNum,
			node->m_CellSize,
			min_center_x,
			max_center_y,
		...
		//	std::cout << node->m_MultipoleCoeff[s * num_of_cell + 61] << std::endl;
		//}
		//std::cout << std::endl;
		//std::cout << std::endl;
		//for(int s=0; s<= tree->m_TermNum; s++){
		//	std::cout << node->m_MultipoleCoeff[s * num_of_cell + 104] << std::endl;
		//}
		//std::cout << std::endl;
	}

	__global__ void Laplace2D_M2M_Kernel1(
		int term_num,
		...
		float2* dMultipoleCoeff)
	{
		...
		unsigned int cell_idx = blockIdx.x * blockDim.x + threadIdx.x;
		unsigned int child_cell_idx = GetChildCellIndex(cell_idx, threadIdx.y);
		...
		for(s=term_num; s>=1; s--){
			...
			for(t=1; t<=s; t++){
				...
				tmp = cuCaddf(
					tmp,
					ucuCmulf(binomial_coefficient(s-1, t-1),
	...
	__global__ void Laplace2D_M2M_Kernel2(
		int term_num,
		float child_cell_size,
		unsigned int num_of_cell,
		unsigned int num_of_ccell,
		...
		float2* dMultipoleCoeff)
	{
		...
		unsigned int child_cell_idx = GetChildCellIndex(cell_idx, threadIdx.x);
	...
	void FMCSM_CUDA2D_Kernel::M2M_Translation(
		int term_num,
		nmc::FMCSM_CUDA2DTreeNode *node)
	{
		cudaError_t err;
		...
		FMCSM_CUDA2DTreeNode* child_node = node->m_pNextTreeNode;
		...
		//Laplace2D_M2M_Kernel1<<<Dg, Db, 4*2*(term_num+1)*block_size*sizeof(float)>>>(
		//	term_num,
		...
		Laplace2D_M2M_Kernel2<<<Dg, Db, 4*2*(term_num+1)*block_size*sizeof(float)>>>(
			term_num,
			child_node->m_CellSize,
			num_of_cell,
			num_of_ccell,
	...
	__global__ void Laplace2D_L2L_Kernel1(
		int term_num,
		float child_cell_size,
		unsigned int num_of_cell,
		unsigned int num_of_ccell,
	...
	void FMCSM_CUDA2D_Kernel::L2L_Translation(
		int  term_num,
		nmc::FMCSM_CUDA2DTreeNode *node)
	{
		int i,j,k;
		...
		dim3 Dg(grid_size, 1, 1);
		dim3 Db(4, block_size, 1);
		Laplace2D_L2L_Kernel1<<<Dg, Db, 4*2*(term_num+1)*block_size*sizeof(float)>>>(
			term_num,
			child_node->m_CellSize,
			num_of_cell,
			num_of_ccell,
	...
	__global__ void Laplace2D_M2L_Kernel1(
		int term_num,
		float cell_size,
		...
		float2* LocalCoeff)
	{
		...
		for(k=0; k<27; k++){
			...
			unsigned int scell_idx = GetCellIndex(idx_x, idx_y);
			...
			for(s=1; s<=term_num; s++){
				...
				for(t=1; t<=term_num; t++){
					...
					L_s = cuCaddf(
						...
						cuCdivf(
							ucuCmulf(
								sign * binomial_coefficient(s+t-1, t-1),
								M_t),
							ucuCpowf(z, s+t)));
				}
	...
	void FMCSM_CUDA2D_Kernel::M2L_Translation(
		int term_num,
		nmc::FMCSM_CUDA2DTreeNode *node)
	{
		int i,j,k;
		...
		dim3 Dg(grid_size, 1, 1);
		dim3 Db(block_size, 1, 1);
		Laplace2D_M2L_Kernel1<<<Dg, Db, 2*2*(term_num+1)*block_size*sizeof(float)>>>(
			term_num,
			node->m_CellSize,
			node_w,
			num_of_cell,
	...
	__global__ void Laplace2D_L2P_Kernel(
		int term_num,
		float cell_size,
		float min_center_x,
		float max_center_y,
	...
	void FMCSM_CUDA2D_Kernel::EvalMultipole(
		const FMCSM_CUDA2DTree* tree,
		const FMCSM_CUDA2DTreeNode *node,
		float2* dst)
	{
		...
		dim3 Dg(grid_size, 1, 1);
		dim3 Db(block_size, 1, 1);
		Laplace2D_L2P_Kernel<<<Dg, Db, 2*(tree->m_TermNum+1)*block_size*sizeof(float)>>>(
			tree->m_TermNum,
			node->m_CellSize,
			min_center_x,
			max_center_y,
	...
	__global__ void Laplace2D_EvalDirect_Kernel(
		int term_num,
		unsigned int node_size,
		const int* startIndexes,
		const int* endIndexes,
	...
	void FMCSM_CUDA2D_Kernel::EvalDirect(
		const FMCSM_CUDA2DTree* tree,
		const FMCSM_CUDA2DTreeNode *node,
		const float2* cf,
		float2* dst)
	{
		...
		dim3 Dg(grid_size, 1, 1);
		dim3 Db(block_size, 1, 1);
		Laplace2D_EvalDirect_Kernel<<<Dg, Db>>>(
			tree->m_TermNum,
			node_size,
			tree->m_pStartIndexes_dev,
			tree->m_pEndIndexes_dev,
...
#endif //_FMCSM_CUDA_LAPLACE_2D_KERNEL_H_