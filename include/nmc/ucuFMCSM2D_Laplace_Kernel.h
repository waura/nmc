#ifndef _NMC_UCU_FMCSM2D_LAPLACE_KERNEL_H_
#define _NMC_UCU_FMCSM2D_LAPLACE_KERNEL_H_
...
namespace nmc
{
	class ucuFMCSM2D_Laplace_Kernel :
	...
	void ucuFMCSM2D_Laplace_Kernel::Init(
		...
		float2* MultipoleCoeff)
	{
		...
		GetCellPos(cell_idx, node_level, &i, &j);
	...
	void ucuFMCSM2D_Laplace_Kernel::ElementMPCefficients(
		const nmc::ucuFMCSM2DTree *tree,
		...
		float2* dMultipoleCoeff)
	{
		...
		unsigned int i,j;
		GetCellPos(cell_idx, node_level, &i, &j);
		unsigned int child_cell_idx = GetChildCellIndex(i, j, node_level, threadIdx.y);
		...
		float2* dMultipoleCoeff)
	{
		...
		unsigned int i,j;
		GetCellPos(cell_idx, node_level, &i, &j);
		unsigned int child_cell_idx = GetChildCellIndex(i, j, node_level, threadIdx.x);
	...
	void ucuFMCSM2D_Laplace_Kernel::M2M_Translation(
		int term_num,
		nmc::ucuFMCSM2DTreeNode *node)
	{
		int node_w, node_h;
	...
	__global__ void Laplace2D_L2L_Kernel2(
		int term_num,
		int node_level,
		float child_cell_size,
		unsigned int num_of_cell,
	...
	void ucuFMCSM2D_Laplace_Kernel::L2L_Translation(
		int  term_num,
		nmc::ucuFMCSM2DTreeNode *node)
	{
		int i,j,k;
		...
		//
		//dim3 Db(4, block_size, 1);
		//Laplace2D_L2L_Kernel2<<<Dg, Db, 4*2*(term_num+1)*block_size*sizeof(float)>>>(
		//	term_num,
		//	node->m_NodeLevel,
		//	child_node->m_CellSize,
		//	num_of_cell,
		...
		float2* LocalCoeff)
	{
		...
		GetCellPos(dcell_idx, node_level, &i, &j);
		...
		for(k=0; k<27; k++){
			...
			unsigned int scell_idx = GetCellIndex(idx_x, idx_y, node_level);
	...
	void ucuFMCSM2D_Laplace_Kernel::M2L_Translation(
		int term_num,
		nmc::ucuFMCSM2DTreeNode *node)
	{
		int i,j,k;
	...
	void ucuFMCSM2D_Laplace_Kernel::EvalMultipole(
		const ucuFMCSM2DTree* tree,
		const ucuFMCSM2DTreeNode *node,
		float2* dst)
	{
	...
	void ucuFMCSM2D_Laplace_Kernel::EvalDirect(
		const ucuFMCSM2DTree* tree,
		const ucuFMCSM2DTreeNode *node,
		const float2* cf,
		float2* dst)
...
#endif //_NMC_UCU_FMCSM2D_LAPLACE_KERNEL_H_