...
#define FMCSM_SOLVER_BiCGSTAB 0
...
public:
	...
	*/
	static SalFMCSM2DTree<VAL_TYPE>* CreateTree(int term_num,
	...
	/*!
		...
		@param[in,out] tree
	*/
	static void DestoryTree(SalFMCSM2DTree<VAL_TYPE>* tree);
	...
	*/
	static void Evaluate(SalFMCSM2DTree<VAL_TYPE>* tree,
		FMCSM2D_Kernel<VAL_TYPE>& kernel, const Vector<VAL_TYPE>& cf,
		Vector<VAL_TYPE>& Ax);
	...
	/*!
		...
		@param[out] x_vec
	*/
	static void Solve(SalFMCSM2DTree<VAL_TYPE>* tree,
		FMCSM2D_Kernel<VAL_TYPE>& kernel,
	...
	/*!
	*/
	static void PlotTreeBox(const SalFMCSM2DTree<VAL_TYPE>* tree);
	static void PlotSourcePoint(const SalFMCSM2DTree<VAL_TYPE>* tree);
	static void PlotBundaryPoint(const SalFMCSM2DTree<VAL_TYPE>* tree);

private:
	static int CreateTreeNode(int term_num,
		SalFMCSM2DTreeNode<VAL_TYPE>* tree_node,
	...
	static int GetMidIndex( Vector2Dd& center_point,
		Vector< FMMPos >& fmm_source_points,
		int find_key, int start_index, int end_index);

	static void SetRootBoxSize(SalFMCSM2DTree<VAL_TYPE>* fmm_tree,
		Vector< FMMPos >& fmm_source_points);

	static void DestoryTreeNode(SalFMCSM2DTreeNode<VAL_TYPE>* node);

	static void UpwardTree(FMCSM2D_Kernel<VAL_TYPE>& kernel, 
		SalFMCSM2DTreeNode<VAL_TYPE>* node, const Vector<VAL_TYPE>& cf);

	static void DownwardTree(FMCSM2D_Kernel<VAL_TYPE>& kernel, 
		SalFMCSM2DTreeNode<VAL_TYPE>* node, const Vector<VAL_TYPE>& cf,
		Vector<VAL_TYPE>& Ax);

	static void DirectEvalution(FMCSM2D_Kernel<VAL_TYPE>& kernel,
		const SalFMCSM2DTreeNode<VAL_TYPE>* from_node,
		const SalFMCSM2DTreeNode<VAL_TYPE>* to_node,
		const Vector<VAL_TYPE>& cf,
		Vector<VAL_TYPE>& Ax);

	static void GetM2LNode(const SalFMCSM2DTreeNode<VAL_TYPE>* src_node,
		SalFMCSM2DTreeNode<VAL_TYPE>** far_parent,
		SalFMCSM2DTreeNode<VAL_TYPE>** far_node);

	static void GetDirectEvalNode(const SalFMCSM2DTreeNode<VAL_TYPE>* src_node,
		SalFMCSM2DTreeNode<VAL_TYPE>** dst_nodes);

	static void PlotTreeNodeBox(const SalFMCSM2DTreeNode<VAL_TYPE>* node);
	static void PlotNodeSourcePoint(const SalFMCSM2DTreeNode<VAL_TYPE>* node);
	static void PlotNodeBundaryPoint(const SalFMCSM2DTreeNode<VAL_TYPE>* node);
};


template<class VAL_TYPE>
SalFMCSM2DTree<VAL_TYPE>* SalFMCSM2D<VAL_TYPE>::CreateTree(
	int term_num,
	const Vector< Vector2Dd > &source_points,
	const Vector< Vector2Dd > &bundary_points)
{
	...
	SalFMCSM2DTree<VAL_TYPE>* fmm_tree = new SalFMCSM2DTree<VAL_TYPE>;
	fmm_tree->m_TermNum = 2*term_num + 1;
	fmm_tree->m_SourceNum = fmm_points.dim();
	fmm_tree->m_RootTreeNode = new SalFMCSM2DTreeNode<VAL_TYPE>;
	fmm_tree->m_RootTreeNode->m_NodeLevel = 1;
	fmm_tree->m_RootTreeNode->m_NodeIndex = 0;
	fmm_tree->m_RootTreeNode->m_ParentNode = NULL;

	SetRootBoxSize( fmm_tree, fmm_points );
	fmm_tree->m_MaxNodeLevel = CreateTreeNode(
		2*term_num+1,
		fmm_tree->m_RootTreeNode,
		fmm_points,
		0,
		fmm_points.dim()-1 );
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::DestoryTree(SalFMCSM2DTree<VAL_TYPE>* tree)
{
	DestoryTreeNode(tree->m_RootTreeNode);
}

template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::Evaluate(
	SalFMCSM2DTree<VAL_TYPE>* tree,
	FMCSM2D_Kernel<VAL_TYPE>& kernel,
	const Vector<VAL_TYPE>& cf,
	Vector<VAL_TYPE>& Ax)
{
...
#endif
	for(i=0; i<4; i++){
		if(tree->m_RootTreeNode->m_NextTreeNode[i]){
			UpwardTree(kernel, tree->m_RootTreeNode->m_NextTreeNode[i], cf);
		}
	}
#ifdef _WINDEBUG
	...
	for(i=0; i<4; i++){
		if(tree->m_RootTreeNode->m_NextTreeNode[i] &&
			tree->m_RootTreeNode->m_NextTreeNode[i]->m_isBundaryPosInNode){
			//init local zero clear
			for(j=0; j<tree->m_RootTreeNode->m_NextTreeNode[i]->m_Local.dim(); j++){
				tree->m_RootTreeNode->m_NextTreeNode[i]->m_Local[j] = 0.0;
			}
			DownwardTree(kernel, tree->m_RootTreeNode->m_NextTreeNode[i], cf, Ax);
		}
	}
#ifdef _WINDEBUG
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::Solve(
	SalFMCSM2DTree<VAL_TYPE>* tree,
	FMCSM2D_Kernel<VAL_TYPE>& kernel,
	int solver_id,
	double& conv_ratio,
	...
	Vector<VAL_TYPE>& x_vec)
{
	...
	for(i=1; i<iteration; i++){
		...
		//calc Ap
		SalFMCSM2D::Evaluate(tree, kernel, p_vec, Ap_vec);
		...
		//calc As
		SalFMCSM2D::Evaluate(tree, kernel, s_vec, As_vec);
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::PlotTreeBox(
	const SalFMCSM2DTree<VAL_TYPE>* tree)
{
	PlotTreeNodeBox( tree->m_RootTreeNode );
}

template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::PlotSourcePoint(
	const SalFMCSM2DTree<VAL_TYPE>* tree)
{
	PlotNodeSourcePoint( tree->m_RootTreeNode );
}

template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::PlotBundaryPoint(
	const SalFMCSM2DTree<VAL_TYPE>* tree)
{
	PlotNodeBundaryPoint( tree->m_RootTreeNode );
}

template<class VAL_TYPE>
int SalFMCSM2D<VAL_TYPE>::CreateTreeNode(
	int term_num,
	SalFMCSM2DTreeNode<VAL_TYPE> *tree_node,
	Vector< FMMPos >& fmm_source_points,
	int start_index,
	int end_index)
{
	...
	if(start_index == end_index){
		if(fmm_source_points[end_index].index < 0){
			FMMPos tmp = fmm_source_points[end_index];
			tmp.index = -tmp.index - 1;
			tree_node->m_pSourcePos = new Vector2D<FMMPos>;
			tree_node->m_pSourcePos->Init(1);
			tree_node->m_pSourcePos->SetVal(0, tmp);
			...
			tree_node->m_pBundaryPos->SetVal(0, fmm_source_points[end_index]);
	...
	if(start_index <= (mid_y_index-1)){
		...
		if(start_index <= (mid_x_index-1)){
			tree_node->m_NextTreeNode[2] = new SalFMCSM2DTreeNode<VAL_TYPE>;
			tree_node->m_NextTreeNode[2]->Init(
				2,
		...
		if(mid_x_index <= (mid_y_index-1)){
			tree_node->m_NextTreeNode[3] = new SalFMCSM2DTreeNode<VAL_TYPE>;
			tree_node->m_NextTreeNode[3]->Init(
				3,
				tree_node->m_NodeLevel+1,
				term_num,
				next_box_length,
	...
	if(mid_y_index <= end_index){
		...
		if(mid_y_index <= (mid_x_index-1)){
			tree_node->m_NextTreeNode[0] = new SalFMCSM2DTreeNode<VAL_TYPE>;
			tree_node->m_NextTreeNode[0]->Init(
				0,
				tree_node->m_NodeLevel+1,
				term_num,
				next_box_length,
		...
		if(mid_x_index <= end_index){
			tree_node->m_NextTreeNode[1] = new SalFMCSM2DTreeNode<VAL_TYPE>;
			tree_node->m_NextTreeNode[1]->Init(
				1,
				tree_node->m_NodeLevel+1,
				term_num,
				 next_box_length,
	...
	tree_node->m_isBundaryPosInNode = false;
	for(i=0; i<4; i++){
		if(tree_node->m_NextTreeNode[i] &&
			tree_node->m_NextTreeNode[i]->m_isBundaryPosInNode){
				tree_node->m_isBundaryPosInNode = true;
				break;
		}
	}
	...
	for(i=0; i<4; i++){
		if(tree_node->m_NextTreeNode[i] &&
			tree_node->m_NextTreeNode[i]->m_isSourcePosInNode){
				tree_node->m_isSourcePosInNode = true;
		}
	}
	return max_node_level;
...
template<class VAL_TYPE>
int SalFMCSM2D<VAL_TYPE>::GetMidIndex(
	Vector2Dd& center_point,
	Vector<FMMPos> &fmm_source_points,
	int find_key,
	int start_index,
	int end_index)
{
	...
	double difs = center_point[find_key] - fmm_source_points[end_index].val[find_key];
	if(dife > difs){
		int i;
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::SetRootBoxSize(
	SalFMCSM2DTree<VAL_TYPE>* fmm_tree,
	Vector<FMMPos>& fmm_source_points)
{
	int i;
	double x, y;
	...
	fmm_tree->m_RootTreeNode->m_BoxLength = (width > height) ? width : height;
	fmm_tree->m_RootTreeNode->m_CenterPos.SetVal(0, min_x + (width/2.0));
	fmm_tree->m_RootTreeNode->m_CenterPos.SetVal(1, min_y + (height/2.0));
}

template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::DestoryTreeNode(SalFMCSM2DTreeNode<VAL_TYPE> *node)
{
	if(node == NULL) return;
	...
	for(i=0; i<4; i++){
		//
		DestoryTreeNode(node->m_NextTreeNode[i]);

		//destory child node
		if(node->m_NextTreeNode[i]){
			delete node->m_NextTreeNode[i];
			node->m_NextTreeNode[i] = NULL;
		}
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::UpwardTree(
	FMCSM2D_Kernel<VAL_TYPE>& kernel,
	SalFMCSM2DTreeNode<VAL_TYPE>* node,
	const Vector<VAL_TYPE>& cf)
{
	assert(node->m_NodeLevel >= 2);
	...
	for(i=0; i<4; i++){
		if(node->m_NextTreeNode[i]){
			UpwardTree(kernel, node->m_NextTreeNode[i], cf);
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::DownwardTree(
	FMCSM2D_Kernel<VAL_TYPE>& kernel,
	SalFMCSM2DTreeNode<VAL_TYPE> *node,
	const Vector<VAL_TYPE>& cf,
	Vector<VAL_TYPE>& Ax)
{
	...
	//calc far node m2l
	SalFMCSM2DTreeNode<VAL_TYPE>* far_parent[5];
	SalFMCSM2DTreeNode<VAL_TYPE>* far_node[7];
	GetM2LNode(node, far_parent, far_node);

	for(i=0; i<5; i++){
		if(far_parent[i] == NULL) continue;
		for(j=0; j<4; j++){
			if(far_parent[i]->m_NextTreeNode[j] &&
				far_parent[i]->m_NextTreeNode[j]->m_isSourcePosInNode){
				kernel.M2L_Translation(far_parent[i]->m_NextTreeNode[j], node);
			}
		}
	}
	for(i=0; i<7; i++){
	...
	if(node->m_pBundaryPos){//this node is leaf
		...
		for(i=0; i<4; i++){
			if(node->m_NextTreeNode[i]){
				for(j=0; j<node->m_NextTreeNode[i]->m_Local.dim(); j++){
					node->m_NextTreeNode[i]->m_Local[j] = 0.0;
				}
			}
		}
		...
		//go to next node
		for(i=0; i<4; i++){
			if(node->m_NextTreeNode[i] && node->m_isBundaryPosInNode){
				DownwardTree(kernel, node->m_NextTreeNode[i], cf, Ax);
			}
		}
	}
}
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::DirectEvalution(
	FMCSM2D_Kernel<VAL_TYPE> &kernel,
	const SalFMCSM2DTreeNode<VAL_TYPE> *from_node,
	const SalFMCSM2DTreeNode<VAL_TYPE> *to_node,
	const Vector<VAL_TYPE>& cf,
	Vector<VAL_TYPE>& Ax)
{
	...
	for(i=0; i<4; i++){
		if(from_node->m_NextTreeNode[i] &&
			from_node->m_NextTreeNode[i]->m_isSourcePosInNode){
			DirectEvalution(
				kernel,
				from_node->m_NextTreeNode[i],
				to_node,
				cf,
				Ax);
		}
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::GetM2LNode(
	const SalFMCSM2DTreeNode<VAL_TYPE> *src_node,
	SalFMCSM2DTreeNode<VAL_TYPE> **far_parent,
	SalFMCSM2DTreeNode<VAL_TYPE> **far_node)
{
	switch(src_node->m_NodeIndex){
		case 0:
			{				
				...
				SalFMCSM2DTreeNode<VAL_TYPE>* tmp;
				tmp = src_node->GetParentNode()->GetUpperLeftNode();
				far_node[0] = (tmp) ? tmp->m_NextTreeNode[0] : NULL;
				far_node[1] = (tmp) ? tmp->m_NextTreeNode[1] : NULL;
				far_node[2] = (tmp) ? tmp->m_NextTreeNode[2] : NULL;
				tmp = src_node->GetParentNode()->GetUpperNode();
				far_node[3] = (tmp) ? tmp->m_NextTreeNode[0] : NULL;
				far_node[4] = (tmp) ? tmp->m_NextTreeNode[1] : NULL;
				tmp = src_node->GetParentNode()->GetLeftNode();
				far_node[5] = (tmp) ? tmp->m_NextTreeNode[0] : NULL;
				far_node[6] = (tmp) ? tmp->m_NextTreeNode[2] : NULL;
			}
			break;
		case 1:
			{
				...
				far_node[2] = (tmp) ? tmp->m_NextTreeNode[0] : NULL;
				far_node[3] = (tmp) ? tmp->m_NextTreeNode[1] : NULL;
				far_node[4] = (tmp) ? tmp->m_NextTreeNode[3] : NULL;
				tmp = src_node->GetParentNode()->GetRightNode();
				far_node[5] = (tmp) ? tmp->m_NextTreeNode[1] : NULL;
				far_node[6] = (tmp) ? tmp->m_NextTreeNode[3] : NULL;
			}
			break;
		case 2:
			{
				...
				tmp = src_node->GetParentNode()->GetLeftNode();
				far_node[0] = (tmp) ? tmp->m_NextTreeNode[0] : NULL;
				far_node[1] = (tmp) ? tmp->m_NextTreeNode[2] : NULL;
				tmp = src_node->GetParentNode()->GetLowerLeftNode();
				far_node[2] = (tmp) ? tmp->m_NextTreeNode[0] : NULL;
				far_node[3] = (tmp) ? tmp->m_NextTreeNode[2] : NULL;
				far_node[4] = (tmp) ? tmp->m_NextTreeNode[3] : NULL;
				tmp = src_node->GetParentNode()->GetLowerNode();
				far_node[5] = (tmp) ? tmp->m_NextTreeNode[2] : NULL;
				far_node[6] = (tmp) ? tmp->m_NextTreeNode[3] : NULL;
			}
			break;
		case 3:
			{
				...
				SalFMCSM2DTreeNode<VAL_TYPE>* tmp;
				tmp = src_node->GetParentNode()->GetRightNode();
				far_node[0] = (tmp) ? tmp->m_NextTreeNode[1] : NULL;
				far_node[1] = (tmp) ? tmp->m_NextTreeNode[3] : NULL;
				tmp = src_node->GetParentNode()->GetLowerNode();
				far_node[2] = (tmp) ? tmp->m_NextTreeNode[2] : NULL;
				far_node[3] = (tmp) ? tmp->m_NextTreeNode[3] : NULL;
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::GetDirectEvalNode(
	const SalFMCSM2DTreeNode<VAL_TYPE> *src_node,
	SalFMCSM2DTreeNode<VAL_TYPE> **dst_nodes)
{
...
template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::PlotTreeNodeBox(
	const SalFMCSM2DTreeNode<VAL_TYPE> *node)
{
	printf("%f %f \n",
	...
	for(i=0; i<4; i++){
		if(node->m_NextTreeNode[i]){
			PlotTreeNodeBox(node->m_NextTreeNode[i]);
		}
	}
}

template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::PlotNodeSourcePoint(
	const SalFMCSM2DTreeNode<VAL_TYPE> *node)
{
	int i;
	if(node->m_pSourcePos){
	...
	for(i=0; i<4; i++){
		if(node->m_NextTreeNode[i]){
			PlotNodeSourcePoint(node->m_NextTreeNode[i]);
		}
	}
}

template<class VAL_TYPE>
void SalFMCSM2D<VAL_TYPE>::PlotNodeBundaryPoint(
	const SalFMCSM2DTreeNode<VAL_TYPE> *node)
{
	int i;
	...
	for(i=0; i<4; i++){
		if(node->m_NextTreeNode[i]){
			PlotNodeBundaryPoint(node->m_NextTreeNode[i]);
		}
	}
}
