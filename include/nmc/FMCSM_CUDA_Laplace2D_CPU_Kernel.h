#ifndef _FMCSM_CUDA_LAPLACE_2D_CPU_KERNEL_H_
#define _FMCSM_CUDA_LAPLACE_2D_CPU_KERNEL_H_
...
namespace nmc
{
	...
	void FMCSM_CUDA2D_CPU_Kernel::Init(
		nmc::FMCSM_CUDA2DTree *tree)
	{
	}

	void FMCSM_CUDA2D_CPU_Kernel::ElementMPCefficients(
		const FMCSM_CUDA2DTree* tree,
		FMCSM_CUDA2DTreeNode *node,
		const float2* c_vec)
	{
	...
	void FMCSM_CUDA2D_CPU_Kernel::M2M_Translation(
		int term_num,
		nmc::FMCSM_CUDA2DTreeNode *node)
	{
		...
		for(j=0; j<node_h; j++){
			for(i=0; i<node_w; i++){
				...
				for(k=0; k<4; k++){
					int child_cell_idx = GetChildCellIndex(cell_idx, k);
	...
	void FMCSM_CUDA2D_CPU_Kernel::L2L_Translation(
		int  term_num,
		nmc::FMCSM_CUDA2DTreeNode *node)
	{
		int i,j,k;
	...
	void FMCSM_CUDA2D_CPU_Kernel::M2L_Translation(
		int term_num,
		nmc::FMCSM_CUDA2DTreeNode *node)
	{
		int i,j,k;
		...
		for(j=0; j<node_h; j++){
			for(i=0; i<node_w; i++){
				...
				unsigned int dcell_idx = GetCellIndex(i, j);
	...
	void FMCSM_CUDA2D_CPU_Kernel::EvalMultipole(
		const FMCSM_CUDA2DTree* tree,
		const FMCSM_CUDA2DTreeNode *node,
		float2* dst)
	{
	...
	void FMCSM_CUDA2D_CPU_Kernel::EvalDirect(
		const FMCSM_CUDA2DTree* tree,
		const FMCSM_CUDA2DTreeNode *node,
		const float2* cf,
		float2* dst)
...
#endif //_FMCSM_CUDA_LAPLACE_2D_CPU_KERNEL_H_